...
class FloPyEnv(gym.Env):
    ...
    def __init__(self,
        ...
        Args:
            ENVTYPE (str): Type of the environment: 0s-c (zero step control), 1s-d, etc.; 
                defaults to '1s-d'.
            PATHMF2005 (str): Absolute path to MODFLOW executable; if None, the default
                distribution is used. Defaults to None.
            PATHMP6 (str): Absolute path to MP6 executable; if None, the default 
                distribution is used. Defaults to None.
            MODELNAME (str): Name of model to use as reference for all files generated by FloPyArcade.
            ANIMATIONFOLDER (str): Folder where animations will be saved.
            _seed (int, optional): Seed used for random number generation. Defaults to None.
            flagSavePlot (bool): Whether or not plots should be saved as a pdf file; defaults to False.
            flagManualControl (bool): Whether the agent can control the particle location 
                at each time step via mouse click/keyboard input. Defaults to False.
            manualControlTime (float, optional): Amount of time that manual control is enabled after a user interaction; 
                defaults to 0.1 s.
            flagRender (bool): Whether or not the animation should be rendered in real-time; defaults to False.
            NAGENTSTEPS (int, optional): Number of steps an agent can take before the environment is reset;
                defaults to 200.
            nLay (int, optional): Number of layers that will make up the model grid; defaults to 1.
            nRow (int, optional): Number of rows that will make up the model grid; defaults to 50.
            nCol (int, optional): Number of columns that will make up the model grid; defaults to 50.
            OBSPREP (str, optional): Method for generating observations: 'perceptron', 'random'; defaults to 'perceptron'.
            initWithSolution (bool, optional): Whether or not the environment should be initialized with a solution; 
                defaults to True.
            PATHMF6DLL (str): Absolute path to MF6 executable if MODFLOW6 is being used as the simulator. Defaults to None.
        """
